...
#HH
DRCpamHH = drm(PAM ~ Temperature, data = pamdata[pamdata$Treatment=="HH",],
                     fct = LL.3(names = c('hill', 'max', 'ed50')))
summary(DRCpamHH)
DRCpamHH$coefficients[3]
ED(DRCpamHH, c(50))

#replicate-specific curve fits
DRCpamHHrep = drm(PAM ~ Temperature, data = pamdata[pamdata$Treatment=="HH",], curveid=Replicate2,
                  fct = LL.3(names = c('hill', 'max', 'ed50')))
summary(DRCpamHHrep)

compParm(DRCpamHHrep, 'ed50')
compParm(DRCpamHHrep, 'ed50', "-")

DRCpamHHrep$coefficients[7:9]
ED(DRCpamHHrep, c(50))[,1]

#Merging Coeffs
Coeffs<-c(DRCpamH0$coefficients[3],DRCpamHH$coefficients[3])
RepCoeffs<-data.frame("ED50"=c(DRCpamH0rep$coefficients[7:9],DRCpamHHrep$coefficients[7:9]),"Treatment"=c(rep("H0",3), rep("HH",3)))

aggregate(ED50 ~ Treatment, data=RepCoeffs, mean)

Coeffs
CoeffsED25<-c(ED(DRCpamH0,c(25))[,1],ED(DRCpamHH,c(25))[,1])

#ED50 ttest
aggregate(ED50 ~ Treatment, data=RepCoeffs, FUN= function(x) shapiro.test(x)$p.value)
bartlett.test(ED50 ~ Treatment, data=RepCoeffs)

ED50.ttest<- t.test(ED50 ~ Treatment, data=RepCoeffs)
ED50.ttest
...
pdf("\\Users\\ralde\\OneDrive - UTS\\Rachel\\Hypoxia_Heat_Konstanz\\ED50_curves\\Currently_used\\CBASS_HH_H0_DRCCurves_CHECK.pdf",10,7)
...
i<-2 #HH
polygon(c(temp_x, rev(temp_x)),c(predict(DRCpamHH, data.frame(Temp = temp_x), interval="confidence")[,2],rev(predict(DRCpamHH, data.frame(Temp = temp_x), interval="confidence")[,3])), col=Colorz[i], density = Denscity)
matpoints(temp_x, predict(DRCpamHH, data.frame(Temperature = temp_x), interval= "confidence"),
          type="l",col=Colorz[i],lty=c(1,3,3),lwd=line_width)
with(pamdata[pamdata$Treatment==Sytes[i],],matpoints(Temperature-offsets[i],PAM,pch=Syms[i], col=Colorz[i], cex=2.5))

legend("bottomleft",c("H0; ED50 = 36.12°C ","HH; ED50 = 35.73°C"),pch=Syms, col=Colorz,pt.cex=2, bty="n",cex=2)
title(main="CBASS T1 H0 vs HH")
